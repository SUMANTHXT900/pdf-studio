...
export function Button({
  ...
  const styles = {
    ...
    danger: 'border border-red-600/30 text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30',
  }[variant];
  return (
    <motion.button
      type={type}
      ...
      <motion.div
        ...
        className={`relative w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm ring-1 transition-colors ${
          over
            ...
            : 'bg-ink-900 dark:bg-paper-100 text-paper-50 dark:text-ink-900 ring-black/5 dark:ring-white/10'
        }`}
      >
        <svg
          width="26"
        ...
        <AnimatePresence>
          {over && (
            <motion.p
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0 }}
              className="text-xs font-semibold text-brass-600 dark:text-brass-300 mt-1.5 tracking-wide uppercase"
            >
              release to add
            </motion.p>
          )}
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0">
          <h1 className="font-display text-2xl sm:text-3xl font-semibold tracking-tight text-ink-900 dark:text-paper-100">
            {name}
          </h1>
          <p className="text-sm text-ink-500 dark:text-ink-300 mt-1 leading-relaxed max-w-xl">
            {desc}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="mt-4" role="status" aria-live="polite">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-xs font-mono tabular-nums text-brass-600 dark:text-brass-300">
          {pct}%
        </span>
      </div>
...
/* Result metadata: subtle user-facing completion lines (counts, size, duration).
   Never shows job ids, timestamps, or event counts — those stay in Testbench. */
export function ResultMeta({ lines }: { lines: Array<string | null | undefined | false> }) {
  const shown = lines.filter((line): line is string => typeof line === 'string' && line.length > 0);
  if (shown.length === 0) return null;
  return (
    <p className="mt-3 text-xs text-ink-400 dark:text-ink-300 tabular-nums">{shown.join(' · ')}</p>
  );
}
...
export function ErrorBlock({
  ...
  const secondary = [engineMessage, details].filter(
  ...
  return (
    <div className="mt-4" role="alert">
      ...
      {code !== undefined && code !== 'CANCELLED' && (
        <p className="mt-1 font-mono text-[11px] text-ink-400/80 dark:text-ink-300/70">{code}</p>
      )}
    </div>
  );
}
      ...
      <div className="mt-3 flex flex-wrap gap-2 pl-11">
        ...
        {shareable && blob && (
          <button
            type="button"
            onClick={async () => {
              const r = await studioShare(blob, name);
